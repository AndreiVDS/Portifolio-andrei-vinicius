export const site = {
  name: 'Andrei Vinícius da Silveira',
  role: 'Full Stack Developer',
  tagline:
    'Full stack developer with a QA and IT-infrastructure background. I build web apps end to end — React on the front, Node / PostgreSQL behind it — and automations that remove manual work.',
  location: 'Jaraguá do Sul, SC — Brazil',
  url: 'https://portifolio-andrei-vinicius.vercel.app',
  email: 'andreivini31@gmail.com',
  cv: '/cv.pdf',
  socials: [
    { label: 'GitHub', href: 'https://github.com/AndreiVDS' },
    { label: 'LinkedIn', href: 'https://www.linkedin.com/in/andrei-vds-dev' },
  ],
}
